...
export function getVerificationTokenFromHeaders (headers: Headers) {
  ...
  return cookieString
    .split(';')
    .find((part) => part.includes('RequestVerificationToken'))
    ?.trim()
    ?.split('=')
    ?.[1];
}

export function createHeaders (sessionId: string, verificationToken: string) {
  return {
